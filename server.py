...
def showCatalog():
    ...
    if 'username' in login_session:
        ...
        return render_template('home.html', catalogs=catalogs, items=items, username=username)
...
def showItem(catalog_id):
    try:
        ...
        if 'username' in login_session:
            ...
            print(username)
            return render_template('category.html', catalogs=catalogs, items=items, catalog=catalog, userid = userid, username=username)
        else:
...
def menuItemDesc(catalog_id, item_id):
    try:
        ...
        if 'username' in login_session:
            ...
            username = getUserInfo(userid).name
            print(username)
            return render_template('item.html', item=item, userid=userid, username=username)
        else:
...
def editCatalog(catalog_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('editCatalog.html', catalog=editCatalog, username = login_session['username'])
...
def deleteCatalog(catalog_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('deleteCatalog.html', catalog=deleteCatalog, username = login_session['username'])
...
def newItem():
    if 'username' not in login_session:
        return redirect('/login')
    if request.method == 'POST':
        newItem = Item(name=request.form['name'],description=request.form['description'] ,catalog_id=request.form['catalog_id'], user_id=login_session['user_id'])
        session.add(newItem)
        try:
            session.commit()
        ...
        userid = getUserId(login_session['email'])
        catalogs = session.query(Catalog).filter_by(user_id=userid).order_by(asc(Catalog.name))
        username = getUserInfo(userid).name
        return render_template('newItem.html', catalogs=catalogs, userid = userid,username=username)
...
def editItem(catalog_id, item_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('editItem.html', item=editItem, catalogs=catalogs, username = login_session['username'])
